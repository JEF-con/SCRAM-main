...
import os
# print(os.getcwd())
from models.Attacker import get_attack_layers_no_acc, pgd_parameter_attack
...
class LocalMaliciousUpdate(object):
    ...
    def train(self, net, test_img = None):
        if self.attack == 'badnet':
            ...
            return self.train_malicious_pgd(net)
        elif self.attack in ('edgecase', 'edge-case', 'normal-edge'):
            # 统一入口调用 Attacker 的 edge-case 实现
            from models.Attacker import train_malicious_edgecase
            return train_malicious_edgecase(net, self.data, self.args)
        else:
    ...
    def train_malicious_dba(self, net, test_img=None, dataset_test=None, args=None):
        # 保存原始全局模型用于计算更新量 (Model Replacement)
        global_model_state = copy.deepcopy(net.state_dict())
        ...
        if self.args.scale > 1.0:
            # L_mal = G_t + (L_trained - G_t) * scale
            trained_state = net.state_dict()
            scaled_state = {}
            for key in trained_state.keys():
                # update = L_trained - G_t
                update = trained_state[key] - global_model_state[key]
                # scaled_update = update * scale
                # New Model = G_t + scaled_update
                scaled_state[key] = global_model_state[key] + update * self.args.scale
            net.load_state_dict(scaled_state)
    ...
    def train_malicious_pgd(self, net, test_img=None, dataset_test=None, args=None):
        """参数空间PGD：对触发器投毒后的批次进行对抗更新，并返回投影后的恶意权重。"""
        net.train()
        poisoned_batches = []
        for images, labels in self.ldr_train:
            images, labels = self.trigger_data(images, labels)
            poisoned_batches.append((images, labels))

        # 直接用批次列表驱动 PGD 参数更新（内部会做投影）
        state_dict, avg_loss = pgd_parameter_attack(
            net, poisoned_batches, self.args,
            eps=getattr(self.args, 'pgd_eps', None),
            proj=getattr(self.args, 'pgd_proj', 'l2'),
            project_frequency=getattr(self.args, 'pgd_project_frequency', 1),
            adv_lr=getattr(self.args, 'pgd_adv_lr', None)
        )
